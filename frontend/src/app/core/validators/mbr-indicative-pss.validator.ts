export class MBRIndicativePssValidator {
  ...
  static getStudyParameterError(parameter: string): string | null {
    ...
    if (!this.STUDY_PARAMETERS.includes(parameter)) {
      return 'Study Parameter must be one of the valid parameter types (1-27)';
    }
    return null;
  }
  ...
  static getPssStudyReferenceError(refId: string): string | null {
