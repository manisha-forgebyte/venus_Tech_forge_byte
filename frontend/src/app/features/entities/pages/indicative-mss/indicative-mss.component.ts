...
import { MBRIndicativeMssValidator } from '../../../../core/validators/mbr-indicative-mss.validator';

@Component({
  selector: 'app-indicative-mss-page',
  standalone: true,
  ...
  template: `
    <div class="page-container">
      <app-top-cards-row
        pageTitle="Indicative MSS"
        pageSubtitle="Indicative MSS list"
        ...
        <div class="middle-actions-row">
          <div class="text-group">
           ...
           <div class="action-buttons-group">
             ...
             <button class="btn btn-darkred-import" (click)="confirmImportOnly()">Import only Indicative MSS</button>
        ...
        <table class="entities-table" *ngIf="!isLoading">
          <thead>
            <tr>
              ...
              <th class="col-params">Study Parameters</th>
          ...
          <tbody>
            <tr *ngFor="let item of filteredData; let i = index">
              ...
              <td class="col-id">{{ item.indicativeMssId || '' }}</td>
        ...
        <div class="modal-content" [class.wide]="modalMode === 'COPY_DATA' || modalMode === 'VIEW'|| modalMode === 'EDIT_PARAM'" (click)="$event.stopPropagation()">
          <div class="modal-header">
            <div class="header-titles">
              ...
              <span class="sub-title" *ngIf="modalMode === 'ADD' || modalMode === 'EDIT'">MBRDB >> {{ modalMode === 'ADD' ? 'Add' : 'Edit' }} Indicative MSS</span>
              <h2 class="main-title">
                {{ modalMode === 'COPY_DATA' ? 'Copy Data to Selected Company' : 
                   modalMode === 'ADD' ? 'Indicative MSS Add' : 'Indicative MSS Edit' }}
              </h2>
            </div>
      ...
      <div class="modal-overlay" *ngIf="studyParametersModalMode" (click)="closeStudyParametersModal()">
        <div class="modal-content study-params-modal" (click)="$event.stopPropagation()">
          <div class="modal-header">
            <div class="header-titles">
              <span class="sub-title">MBRDB >> Indicative MSS</span>
              <h2 class="main-title">Indicative MSS Study Parameters</h2>
            </div>
          ...
          <div class="modal-body">
            <app-skeleton-loader *ngIf="isParamsLoading" type="table" [rows]="4" [columns]="5"></app-skeleton-loader>
              ...
              <table class="study-params-table">
                ...
                <tbody>
                  <tr *ngFor="let param of studyParametersData; let i = index" [class.alt-row]="i % 2 === 1">
                    <td class="col-season">{{ param.season }}</td>
  ...
  `,
  styleUrls: ['./indicative-mss.component.scss']
})
export class IndicativeMssComponent implements OnInit {
  data: any[] = [];
  isLoading = false;
  errorMessage = '';
  ...
  editingIndicativeMssId: number | null = null;
  ...
  get filteredData() {
    return this.data.filter(item => {
      ...
      const fercMatch = !this.filterFercId || String(item.indicativeMssId || '').toLowerCase().includes(this.filterFercId.toLowerCase());
      ...
      const seasonMatch = !this.filterSeason || String(item.season || '').toLowerCase().includes(this.filterSeason.toLowerCase());
      
      return globalMatch && fercMatch && typeMatch && yearMatch && baMatch && scenarioMatch && seasonMatch;
    });
  }
  ...
  currentStudyParamsPid = 0;
  currentStudyParamsId = 0;
  ...
  loadData() {
    ...
    this.apiService.getIMSSListByCID(this.companyId).subscribe({
      next: (response: any) => {
      ...
      error: (err: any) => {
        console.error('Error loading indicative MSS:', err);
        this.errorMessage = 'Failed to load indicative MSS. Please try again.';
        this.isLoading = false;
      }
    });
  }
  ...
  openModal(mode: string) { 
    if (mode === 'ADD') {
      ...
      this.editingIndicativeMssId = null;
    }
    this.modalMode = mode; 
  }
  ...
  saveData() {
    ...
    const validation = MBRIndicativeMssValidator.validateRecord(this.formData);
    ...
    const payload: any = {
      ...
      pid: targetPid,
      cid: this.companyId,
      indicative_mss_id: isEditMode ? this.editingIndicativeMssId : null,
      indicative_mss_study_id: null,
    ...
    console.log('[IMSSsaveData] Submitting payload:', JSON.stringify(payload, null, 2));

    this.apiService.insUpdIMSSUIWithResponse(payload).subscribe({
      next: (res: any) => {
        console.log('Saved indicative MSS - Response Status:', res.status, 'Body:', res.body);
        if ([200, 201, 204].includes(res.status)) {
          this.toast.success(this.modalMode === 'ADD' ? 'MSS added.' : 'MSS updated.');
          this.closeModal();
      ...
      error: (err: any) => {
        console.error('Error saving indicative MSS:', err);
  ...
  onEdit(item: any) {
    ...
    this.editingIndicativeMssId = r.indicative_mss_id ?? null;
    this.modalMode = 'EDIT';
  }
  ...
  async onDelete(item: any) { 
    if (await this.confirmService.show('Delete this record?', 'Confirm Delete', 'Delete', 'Cancel')) { 
      const r = item.raw ?? item; 
      this.apiService.deleteIMSSByID(r.pid ?? 0, r.indicative_mss_id ?? 0).subscribe({
        next: () => { 
  ...
  openStudyParameters(item: any) {
    ...
    this.currentStudyParamsId = r.indicative_mss_id ?? 0;
  ...
  loadStudyParameters() {
    ...
    this.apiService.getIMSSParamsListByCIDAndId(this.companyId, this.currentStudyParamsPid).subscribe({
      next: (response: any) => {
        ...
        this.studyParametersData = records.map((r: any) => ({
          pid: r.pid ?? this.currentStudyParamsPid,
          cid: r.cid ?? this.companyId,
          season: r.Season_desc ?? r.season_desc ?? r.season ?? 'N/A',
          paramNo: r.study_parameter ?? r.Study_parameter ?? '',
          paramName: r.study_param ?? r.Study_param ?? '',
          paramValue: r.study_parameter_value ?? r.study_parameter_value ?? '',
          paramReference: r.study_parameter_reference ?? r.study_parameter_reference ?? '',
          raw: r
        }));
      ...
      error: (err: any) => {
        ...
        this.toast.error('Failed to load study parameters. Please try again.');
        this.isParamsLoading = false;
      }
    });
  }
  ...
  async saveStudyParameters() {
    ...
    this.isLoading = true;
    const payload = this.studyParametersData.map((p: any) => ({
      pid: p.pid,
      cid: p.cid,
      season: p.season,
  ...
  toggleAll(e: Event) {
    ...
    this.apiService.updateIncInFilingFlagAll(this.companyId, {
      table: 'indicative_mss', tableId: 'pid', value: checked ? '1' : '0'
  ...
  onCheckboxChange(item: any) {
    ...
    this.apiService.updateIncInFilingFlag(this.companyId, {
      table: 'indicative_mss', tableId: 'pid', value: item.selected ? '1' : '0', whereIds: String(pid)
