...
async function main() {
  ...
  for (const role of roles) {
    await prisma.role.upsert({
      where: { gid: role.gid },
      update: { name: role.name, rolename: role.rolename, isActive: true },
      create: role,
    });
  }
  ...
  await prisma.accountGroup.upsert({
    where: { agid: 1 },
    update: { aid: 1, gid: 1, name: 'Default Group', groupname: 'Default Group', isActive: true },
    create: { agid: 1, aid: 1, gid: 1, name: 'Default Group', groupname: 'Default Group' },
  });
  ...
  await prisma.companyFilingFlags.upsert({
    where: { cid: 1 },
    update: { includeAssets: true, includeEntities: true, includeFilings: true },
    create: { cid: 1, includeAssets: true, includeEntities: true, includeFilings: true },
  });
  ...
  for (const lookup of lookups) {
    ...
    if (existing) {
      await prisma.commonLookup.update({ where: { id: existing.id }, data: { isActive: true, aid: lookup.aid ?? null } });
    } else {
      await prisma.commonLookup.create({ data: lookup });
    }
  }

  await prisma.user.upsert({
    where: { email: adminEmail },
    update: { name: 'Local Admin', password, role: 'Site Admin', aid: 1, cid: 1, gid: 1, isActive: true },
...
main()
  ...
  .catch(async (error) => {
    console.error('Runtime seed error', error);
    await prisma.$disconnect();
    process.exit(1);
  });
